myMedBookController.controller('gestioneRichiesteCOCController', function ($scope, $http, $window, $stateParams, $rootScope, $mdDialog, $timeout, $q, $location, $cookies, $interval) {
    $('.mobile_MMB').hide()
    $('.mobile_CDV').show()
    ...
    $scope.getAlarms = function (flag) {
        ...
        $http.get(BASE_URL + 'COC/assistance/list/?structure_id=' + structure_id + query_string).then(function successCalback(response) {
            $scope.alarms = response.data;
            if (!pause && !flag && $scope.alarms.length > 0) {
    ...
    $scope.onoffAlarmCOC = function (alarm) {
        $http.post(BASE_URL + 'COC/assistance/toggleactive/?assistance_id=' + alarm.pk, { 'handled': alarm.handled }).then(function successCallback(response) {
            if ($scope.flag_handled)
                $scope.getAlarms(true);
            else
                $scope.getAlarms(false);
    ...
    $scope.downloadPDF = function () {
        $scope.list = []
        $http.get(BASE_URL + 'COC/assistance/filter/').then(function (response) {
            ...
            var subcontent = {
                ...
                table: {
                    ...
                    body: [
                        ...
                        { text: 'Coordinate', style: 'tableHeader', alignment: 'center' },
            ...
            angular.forEach($scope.allAlarm, function (alarm, idx) {
                var data = [
                    ...
                    { 'text': alarm.affiliation.user.first_name + ' ' + alarm.affiliation.user.last_name, style: 'row', alignment: 'center' },
                    { 'text': alarm.affiliation.user.email, style: 'row', alignment: 'center' }
                ]

                if (alarm.latlng)
                    data.push({ 'text': alarm.latlng.coordinates[0] + ', ' + alarm.latlng.coordinates[0], style: 'row', alignment: 'center' })
                else
